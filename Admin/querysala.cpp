#include "querysala.h"
#include "ui_querysala.h"

QuerySala::QuerySala(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::QuerySala)
{
    ui->setupUi(this);
...
void QuerySala::update_table(const std::vector<Salary> &salaries)
{
    // 先清理所有的已有数据
...
void QuerySala::on_query_btn_clicked()
{
    std::vector<Salary> result;
    QString id = ui->id_edit->text();
    if (id.isEmpty()) {
        QMessageBox::warning(this, "错误", "请输入要查询人员的ID！", QMessageBox::Ok);
        return;
    }
